...
export const enableLoupe = (
  ...
  const handler = function () {
    ...
    zoomingImg.onload = () => {
      ...
      const left =
        targetRect.left +
        (wnd?.pageXOffset || doc.documentElement.scrollLeft) -
      ...
      const loupeOffsetX = loupe.elem.getBoundingClientRect().width / 2;
      const loupeOffsetY = loupe.elem.getBoundingClientRect().height / 2;

      moveHandlers.docMouseMoveHandler = function (e: MouseEvent) {
        if (
          e.pageX < left - exitDistance ||
        ...
        const zoomingPercentX =
          (width * loupe.magnification * percentX) / 100 + loupeOffsetX;

        const positionY = -e.pageY + top;
        const percentY = (positionY * 100) / targetRect.height;
        const zoomingPercentY =
          (height * loupe.magnification * percentY) / 100 + loupeOffsetY;
